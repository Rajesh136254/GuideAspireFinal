...
@Component({
  ...
  templateUrl: './class6-10.html',
  styleUrls: ['./class6-10.css'],
  encapsulation: ViewEncapsulation.None
})
export class Class6_10Component implements OnInit, AfterViewInit {
  // Template references
  ...
  // Component state
  selectedClass = 6; // Default to class 6
  selectedDay = 1;
  ...
  ngOnInit(): void {
    // Initialize progress for all classes (6-10)
    for (let i = 6; i <= 10; i++) {
      this.progress[i] = [];
    }
  ...
  // All the other methods from Class1_5Component remain the same
  // Mobile menu toggle
  ...
  async fetchUserProfile(): Promise<void> {
    try {
      ...
      if (Array.isArray(profile.progress)) {
        this.progress[6] = profile.progress.map(Number).filter(n => !isNaN(n));
      }
    } catch (error: any) {
  ...
  async markDayComplete(): Promise<void> {
    ...
    try {
      ...
      // Refresh progress from the server to ensure consistency
      await this.fetchProgress();
